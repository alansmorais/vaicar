...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleDeletePassenger = async () => {
    ...
    try {
      setIsUpdating(true);
      const res = await deletePassenger(deletingPassenger.id);
      const deletedName = deletingPassenger.name;
      // Immediately remove only the selected passenger from the list
      setPassengers(prev => prev.filter(p => p.id !== deletingPassenger.id));
      setDeletingPassenger(null);
      setDeleteConfirmation('');
      setDeleteSuccess(res.message || `Conta do passageiro "${deletedName}" excluída com sucesso do banco de dados.`);
      setTimeout(() => setDeleteSuccess(''), 7000);
    } catch (err: any) {
      setDeleteError(err.message || 'Erro ao excluir passageiro no banco de dados.');
    } finally {
      setIsUpdating(false);
    }
  };
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      {tab === 'PASSENGERS' && (
        <div className="space-y-6">
          ...
          {/* Success Banner */}
          {deleteSuccess && (
            <div className="p-4 bg-emerald-500/15 border border-emerald-500/30 rounded-2xl flex items-center gap-3 text-emerald-400 text-xs font-bold animate-in fade-in">
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800 bg-slate-950/40 text-slate-400 font-bold uppercase tracking-wider">
                      ...
                      <th className="p-4 text-right">Excluir Passageiro</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={p.id} className="hover:bg-slate-950/20 transition-colors">
                        ...
                        <td className="p-4 text-right">
                          <button
                            ...
                            className="px-2.5 py-1.5 rounded-lg font-bold text-[11px] bg-rose-950/60 hover:bg-rose-900 text-rose-300 hover:text-white transition-all cursor-pointer border border-rose-800/80 inline-flex items-center gap-1.5 ml-auto"
                            title={`Excluir conta do passageiro ${p.name}`}
                          >
                            <Trash2 className="w-3.5 h-3.5" />
            ...
            <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-slate-900 border border-rose-500/30 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl animate-in zoom-in-95">
                ...
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                  ...
                  <p className="text-[11px] text-slate-400">
                    Verifique os dados da conta do passageiro selecionado antes de confirmar a exclusão:
                  </p>

                  <div className="pt-2 border-t border-slate-900 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-slate-500 block">Nome do Passageiro</span>
                      <strong className="text-white font-bold">{deletingPassenger.name}</strong>
                    </div>
                    <div>
                      <span className="text-[10px] uppercase font-bold text-slate-500 block">WhatsApp / Telefone</span>
                      <strong className="text-emerald-400 font-mono font-bold">{deletingPassenger.phone}</strong>
                    </div>
                    <div>
                      <span className="text-[10px] uppercase font-bold text-slate-500 block">E-mail</span>
                      <span className="text-slate-300">{deletingPassenger.email || 'Não informado'}</span>
                    </div>
                    <div>
                      <span className="text-[10px] uppercase font-bold text-slate-500 block">ID da Conta</span>
                      <span className="text-slate-400 font-mono text-[10px]">{deletingPassenger.id}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[11px] text-amber-300 flex items-start gap-2">
                  ...
                  <span>
                    Apenas esta conta de passageiro selecionada será excluída permanentemente. Motoristas, outros passageiros e históricos de corridas permanecem intactos.
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>{isUpdating ? 'Excluindo...' : 'Confirmar Exclusão'}</span>
                  </button>
                </div>
              </div>
            </div>
