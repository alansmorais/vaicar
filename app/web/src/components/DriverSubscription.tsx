...
export const DriverSubscription: React.FC<DriverSubscriptionProps> = ({
  ...
  const handleSimulatePayment = () => {
    ...
    setTimeout(() => {
      ...
      alert(
        isFreePlan
          ? '🎉 Parabéns! Sua isenção de R$ 0/mês como 1º motorista registrado está ativa.'
          : `Plano mensal ativado com sucesso por R$ ${activeFee}/mês!`
      );
    }, 600);
  };
    ...
    <div className="w-full max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-1.5 bg-emerald-950/80 border border-emerald-500/30 px-3 py-1 rounded-full">
          ...
          <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider">
            Planos Comerciais VaiCar
          </span>
        </div>
        <h1 className="text-3xl font-black text-white">Plano Mensal do Motorista</h1>
        <p className="text-slate-300 text-sm max-w-md mx-auto">
          Escolha entre 10% por corrida ou plano mensal de R$100.
        </p>
      </div>
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950/30 border-2 border-emerald-500/60 rounded-3xl p-7 shadow-2xl space-y-6 relative overflow-hidden">
        ...
        <div className="space-y-3 pt-2">
          ...
          {[
            'Receba solicitações de passageiros locais e turistas em São Sebastião',
            'Perfil profissional verificado com foto e dados do veículo',
            'Defina seus próprios preços e rotas de atendimento',
            'Defina sua área de atuação e adicione zonas personalizadas',
            'Comunicação direta com o passageiro via aplicativo e WhatsApp',
          ...
          >
            ...
            <span>
              {isProcessing
                ...
                : driver.subscriptionStatus === 'ACTIVE'
                ? `RENOVAR PLANO MENSAL (R$ ${activeFee}/MÊS)`
                : `ASSINAR PLANO MENSAL (R$ ${activeFee}/MÊS)`}
            </span>
          </button>
          <p className="text-[10px] text-center text-slate-500 mt-2">
            Modelos comerciais disponíveis: Escolha entre 10% por corrida ou plano mensal de R$100.
          </p>
        </div>
      </div>
    </div>
